...
const MAX_ITEMS = 24 // bound memory — keep only the most recent shares
const store = new Map() // id -> { mp4, poster, title, createdAt }

// Serialize transcodes so two concurrent shares can't double the memory spike.
...
function sweep() {
  ...
  // Hard cap (oldest first — Map preserves insertion order).
...
async function transcode(webm) {
  ...
  try {
    // Scale to 720×1280 and pin threads — keeps the encoder's memory/CPU low
    // enough to stay inside the pod limit while staying sharp for Stories.
    await run('ffmpeg', [
      '-y', '-threads', '1', '-i', inPath,
...
export async function putShare(webmBuffer, title) {
  sweep()
  const { mp4, poster } = await enqueue(() => transcode(webmBuffer))
