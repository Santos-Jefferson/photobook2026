...
export async function getNarrationAudio({ text, lang = 'en', shouldTranslate = true, env = process.env }) {
  ...
  try {
    ...
    // Network-level failure (often a corporate proxy/firewall, or TLS MITM cert
    // Node doesn't trust). Surfaced clearly so it's not mistaken for a bad key.
    const e = new Error('Could not reach ElevenLabs (network/proxy/TLS): ' + (err.cause?.code || err.message))
    e.code = 'NETWORK'
    throw e
  }
