...
- **Voice narration + translation** — the language switch (**English / Portuguese / Spanish**) translates the **on-screen text** *and* narrates it in a **natural neural voice** (Microsoft Edge "Read Aloud", free, no API key) via the `/api/narrate` + `/api/translate` server. Reads opening → each frame → closing. Falls back to the browser's built-in voice when the server isn't running (e.g. the static export).
